...
%token mult div plus minus lt gt
...
%%
Program		: FunctionList {root = CN1N(program_n,$1);}
    		;
...
ParameterList	: VariableList {$$ = CN1N(parameter_list_n,$1);}
		...
		;
Function	: FUNC Variable '(' ParameterList ')' Statement {$$ = CN3N(function_list_n,$2,$4,$6);}
		;
Statement 	: AssignmentStatement {$$ = CN1N(statement_n,$1);}
		...
		| BreakStatement {$$ = CN1N(statement_n,$1);}*/ 
		| Block {$$ = CN1N(statement_n,$1);}
		;
Block		: '{' DeclarationList StatementList '}' {$$ = CN2N(block_n,$2,$3);}
		;
...
ForStatement	: FOR AssignmentStatement TO Expression DO Statement DONE {$$ = CN3N(for_statement_n,$2,$4,$6);}
		;
Expression 	: Expression plus Expression {$$ = CN2D(expression_n,plus,$1,$3);} 
		| Expression minus Expression {$$ = CN2D(expression_n,minus,$1,$3);} 
		| Expression mult Expression {$$ = CN2D(expression_n,mult,$1,$3);}
		| Expression div Expression {$$ = CN2D(expression_n,div,$1,$3);}
		| Expression lt Expression {$$ = CN2D(expression_n,lt,$1,$3);}
		| Expression lt Expression {$$ = CN2D(expression_n,lt,$1,$3);}
		| Expression EQUAL Expression {$$ = CN2D(expression_n,EQUAL,$1,$3);}
		| Expression NEQUAL Expression {$$ = CN2D(expression_n,NEQUAL,$1,$3);}
		| Expression LEQUAL Expression {$$ = CN2D(expression_n,LEQUAL,$1,$3);}
		| Expression GEQUAL Expression {$$ = CN2D(expression_n,GEQUAL,$1,$3);}
...
Declaration	: VAR VariableList {$$ = CN1N(declaration_n,$2);}
		;
Variable 	: IDENTIFIER {char* data = STRDUP(yytext); $$ = CN0D(variable_n,data);}
		;
Integer		: NUMBER {int* data = (int*) malloc(sizeof(int)); *data = atoi(yytext); $$ = CN0D(integer_n,data);}
		;
PrintItem	: Expression {$$ = CN1N(print_item_n,$1);}
		| Text {$$ = CN1N(print_item_n,$1);}
		;
Text		: STRING {char* data = STRDUP(yytext); $$ = CN0D(text_n,data);}
		;
